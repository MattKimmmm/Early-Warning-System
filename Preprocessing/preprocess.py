...
from utils import unique_column, common_patients, load_np, aggregate_events_for_item_bin
...
def aggregate_events_output_lab(keyword):
    ...
    for patient in patient_output:
        args = (patient, df_output_unique, 1)
        args_list_output.append(args)

    with Pool(processes=8) as pool:
        results = pool.map(patient_specifics_bin, args_list_output)
    ...
    with Pool(processes=8) as pool:
        results = pool.map(patient_specifics_bin, args_list_lab)
...
# aggregate events into hourly bins
def aggregate_events_output_lab_bin(keyword, num_bins):
    # load dataframes
    df_patients = load_from_csv(f'patients_{keyword}_neg')
    # [df_chart_event, df_output_event, df_lab_icu_specific] = load_from_csv_multiple([f'chart_events_{keyword}', f'output_events_{keyword}', f'lab_icu_specific_{keyword}'])
    [df_output_event, df_lab_icu_specific] = load_from_csv_multiple([f'output_events_{keyword}_neg', f'lab_icu_specific_{keyword}_neg'])
    ...
    print(f"Input dimensions: {num_patients} patients x {num_hours} hours x {num_items} items")
    # print(input)
    save_to_npy(input_output, f'input_{keyword}_output_{num_bins}bin')
...
def aggregate_events_output_lab_bin_negative(keyword, num_bins):
    ...
    print(f"Input dimensions: {num_patients} patients x {num_hours} hours x {num_items} items")
    # print(input)
    save_to_npy(input_output, f'input_{keyword}_output_{num_bins}bin_neg')
    ...
    print(f"Input dimensions: {num_patients} patients x {num_hours} hours x {num_items} items")
    # print(input)
    save_to_npy(input_lab, f'input_{keyword}_lab_{num_bins}bin_neg')
        ...
        if times_df.empty:
            ...
            for item_df in items_dfs:
                ...
                if item_df.empty:
                    ...
                    item_name = item_df['ITEMID'].iloc[0]
                    # print(f"Columns in item_df: {item_df.columns}")
                    item_df_numeric = pd.to_numeric(item_df["VALUE"], errors='coerce')
                    # print(f"item_df_numeric:\n{item_df_numeric}")
                    quantile_values = []
...
# given keyword, load processed dataset and create dataloaders
def create_dataset_bin(keyword, batch_size, random_seed, num_bins) :
    # [# patients X 72 hours X # items]
    lab_name = f"input_{keyword}_lab_{num_bins}bin.npy"
    output_name = f"input_{keyword}_output_{num_bins}bin.npy"

    lab_name_neg = f"input_{keyword}_lab_{num_bins}bin_neg.npy"
    output_name_neg = f"input_{keyword}_output_{num_bins}bin_neg.npy"
    ...
    # save tensors and dataloader params
    torch.save(data_train_tensor, os.path.join(file_path, f'data_train_tensor_{keyword}_{num_bins}bin.pt'))
    torch.save(labels_train_tensor, os.path.join(file_path, f'labels_train_tensor_{keyword}_{num_bins}bin.pt'))
    torch.save(data_test_tensor, os.path.join(file_path, f'data_test_tensor_{keyword}_{num_bins}bin.pt'))
    torch.save(labels_test_tensor, os.path.join(file_path, f'labels_test_tensor_{keyword}_{num_bins}bin.pt'))
